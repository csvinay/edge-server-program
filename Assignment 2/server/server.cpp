...
string get_data_string(string file_name) //receives name of file as the input and returns a string with all text
{
...
vector<int> get_white_spaces(string sentence) //returns a vector as index of white spaces , called by tokeniser
{
...
vector<string> tokenizer(string sentence) // tokenize a given string
{
	int p;
...
int send(int file_descriptor,string message,int count){
	...
	if(n<0){
		cout<<"\nERROR: Couldn't write to the port!\n"<<endl;
		}
	return n;
}
...
string recieve(int file_descriptor, int count){
	...
	if(n<0){
		cout<<"\nERROR: Couldn't read from the socket!\n"<<endl;
	}	
	string stri(buffer);	
	return stri;
}
...
void map_command(vector <string> command_args,int file_descriptor){
	...
	if(command_args.at(0) == "ls"){
		...
		if(n<0){
			cout<<"\nERROR: Couldn't send the file description.\n"<<endl;
		}
	}
...
void turn_on_server(string server_type){
	...
	if(file_descriptor < 0){
		...
		if(status < 0){
			...
			if(status != 0){
				cout<<"\nERROR: Failed to get to listen mode\n"<<endl;
			}
			else{
				cout<<"\nServer has started! Please make request from client side to connect."<<endl;												
